...
export async function register(data: { name: string; phone: string; email?: string }) {
...
export async function verifyOtp(phone: string, code: string) {
  ...
  if (!otpRecord) throw new Error('Código inválido.');
  if (otpRecord.expiresAt < new Date()) throw new Error('Código expirado. Solicita um novo.');
...
export async function requestLoginOtp(phone: string) {
  ...
  if (!user) throw new Error('Número não registado.');
  if (user.status === 'SUSPENDED') throw new Error('Conta suspensa. Contacta o suporte.');
